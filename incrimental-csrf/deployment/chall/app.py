...
app.config['secret'] = 123456789123456789
...
def home(user):
    ...
    app.config['secret'] += 666
    token = base64.b64encode(hex(app.config['secret']).encode()).decode()
    session['token'] = token
    return render_template("home.html",username=session.get("user"),post_list=result,access=access,user=user,token=token)
